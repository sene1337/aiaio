...
const compiled = spawnSync(tsc, [
  ...
  'src/levels.ts', 'src/history.ts', 'src/campaign.ts', 'src/session-director.ts',
  'tests/levels.test.ts', 'tests/history.test.ts', 'tests/campaign.test.ts',
], { stdio: 'inherit' });
if (compiled.status !== 0) process.exit(compiled.status ?? 1);

for (const testFile of ['levels.test.js', 'history.test.js', 'campaign.test.js']) {
